                        ...
                        <div class="sl-item">
                            ...
                            <div class="sl-content">
                                ...
                                <div class="sl-author font-weight-bold size-6">
                                    <a style="color: red" href="{{route('profile',$report->user->id)}}">{{"from:"}}{{$report->user->name}}{{" I am reporting this post because of " }}{{$report->note}}</a>
                                </div>
                                <div class="sl-author font-weight-bold size-6">
                                    <a style="color: #6f2877" href="">{{$report->post->user->name}}</a>
                                </div>
                                <div class="sl-date text-muted">{{$report->post->created_at->diffForHumans()}}</div>

                                <div class="sl-author text-center font-weight-bold h3">
                                    <a href="">{{$report->post->title}}</a>
                                    <div>
                                        @foreach($report->post->allTags() as $tag)
                                ...
                                @if($report->post->image)
                                    <div class="d-flex justify-content-center align-items-center" style="max-width: 450px ;max-height: 400px ;margin-top: 50px; margin-bottom:40px " >
                                        <div class="text-center">
                                            <img  src="{{asset($report->post->image)}}" width=100% height="auto" >
                                        </div>
                                    </div>
                                @endif
                                ...
                                @if($report->post->comments != null)
                                    ...
                                    @endforeach
                                @endif
                            <a class="btn btn pull-right " style="background-color: silver; color:white" href="{{route("approve",$report->id)}}">approve</a>
                            <a  class="btn btn-danger pull-right mr-2 " href="{{route("reject",$report->id)}}">reject</a>
